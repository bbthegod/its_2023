...
export default function UserDetail() {
  ...
  const {
    ...
    // error: playError,
    data: playData,
  } = useQuery(['play'], () => query(`/play/user/${id}`), {
    enabled: !!id,
  });
  const play = playData?.data

  const update = useMutation((data: User) => mutation(`/user/${id}`, data, 'PUT'));
  const remove = useMutation(() => mutation(`/user/${id}`, null, 'DELETE'));
  const reset = useMutation(() => mutation(`/play/${id}`, null, 'DELETE'));
  ...
  //====================================== Render ======================================
  if (userLoading || playLoading) return null;
  return (
    <div>
      <Header subtitle="Người Dùng" title={user ? user.studentName : ''} />
      <div className="tabs mt-6">
        <a className={`tab tab-bordered ${tab === 0 ? "tab-active" : ""}`} onClick={() => setTab(0)}>SINH VIÊN</a>
        <a className={`tab tab-bordered ${tab === 1 ? "tab-active" : ""}`} onClick={() => setTab(1)}>VÒNG CHƠI</a>
      </div>
      <div className="mt-6">
        {tab === 0 && user && (
