...
export default function QuestionDetail() {
  ...
  const {
    ...
    data: questionData,
    refetch,
  } = useQuery(['question'], () => query(`/question/${id}`), {
    enabled: !!id,
  });
  const question = questionData?.data

  const update = useMutation((data: Question) => mutation(`/question/${id}`, data, 'PUT'));
  const remove = useMutation(() => mutation(`/question/${id}`, null, 'DELETE'));
